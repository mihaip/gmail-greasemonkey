...
function editPersistentSearches(event) {
  ...
  innerContainer.innerHTML += 
    '<p>Use <a href="https://mail.google.com/support/bin/answer.py?answer=7190&topic=1579" target="_blank">operators</a> ' +
...
PersistentSearch.prototype.getListItem = function() {
  if (!this.listItem) {
    ...
    if (this.type == PersistentSearch.SAVED_TYPE) {
      this.listItem.id = SAVED_SEARCH_PREFIX + this.label;
    }    
    this.listItem.className = "lk cs listItem";
    this.listItem.appendChild(newText(this.label));
    this.listItem.addEventListener("mousedown", 
