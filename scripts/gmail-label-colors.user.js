...
// These are actually dynamic, and Firefox seems to crash if we initialize them
// inline, so they are set in overrideEscape()
var NO_ESCAPE_START;
var NO_ESCAPE_END;
...
function overrideEscape() {
  // We want the no escape start/end markers to be non-deterministic, so that 
  // an attacker can't embed arbitrary HTML into an evil message. The action token
  // seems like a good secret string.
  NO_ESCAPE_START = '<!-- ' + getCookie("GMAIL_AT") + ' NES -->';
  NO_ESCAPE_END = '<!-- ' + getCookie("GMAIL_AT") + 'NEE -->';
